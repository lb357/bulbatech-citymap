...
| GET | /api/feed | ?page=int&category=int | | {"page": int, "category": int, "tickets": [{see /api/ticket, likes&dislikes->rating }, ...]} | |
| GET | /api/ticket | ?ticket_id=int | | {"ticket_id": int, "icon": str(UNICODE), "title": str, "category": int, "text": str, "files_json": [str(url), ...] "user_id": int, "fullname": str, "likes": int, "dislikes": int, "official_comment": str, "comments": [ {"user_id": int, "text": str}, ... ], "point": [float, float], "pinned": bool, "timestamp": int, "archive": bool} | |
| GET | /api/points | | | {"points": [ [[float, float] (point), int (ticket_id)], ...]} | |
| GET | /api/tickets | ?page=int | | {"page": int, "user_id": int, "tickets": [{see /api/ticket, likes&dislikes->rating }, ...]} | |
